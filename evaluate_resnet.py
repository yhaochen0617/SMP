...
def calculate_metric(dataloader, model, device, atc_thre, fid_mean, fid_cov, candidate, stage, save_root, metrics=['entropy', 'maxconf', 'atc', 'fid', 'stat']):
    ...
    if os.path.isfile(logits_path) and os.path.isfile(feat_path):
        ...
        for batch in iter(dataloader):
            ...
            if 'atc' in metrics:
                atc_scores.extend((atc_thre-ep).tolist())
...
if __name__ == "__main__":
    ...
    if args.load_model != '' and os.path.isfile(args.load_model):
        ...
        decay = []
        no_decay = []
        optimizer = torch.optim.SGD(params_list.parameters(), lr=base_lr, momentum=0.9, weight_decay=5e-4)
        ...
        for iters in range(max_iter):
            ...
            optimizer.zero_grad()
            pred = method(x)
            loss = F.mse_loss(pred.reshape(-1), y)
    ...
    for val_s in val_metrics:
        ...
        with torch.no_grad():
            val_y_hat = method(val_x).cpu().numpy()
    ...
    for k,v in val_pred.items():
        ...
        if rmse_loss < best_loss:
            ...
            best_loss = rmse_loss
    print('*'*20)
    print('NOTE: The RMSE on val set should be in [3.1, 3.2] when threshold is set to 0.5. If False, please try again!')
    print('*'*20)
    print("===> Successing! ")
